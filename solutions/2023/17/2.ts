...
function solve() {
  ...
  while (remaining.length > 0) {
    ...
    if (left && dirStepsTaken >= 4 && !seen.has([left.row, left.col])) {
    ...
    if (right && dirStepsTaken >= 4 && !seen.has([right.row, right.col])) {
      const seenClone = seen.clone();
    ...
    if (forward && dirStepsTaken < 10 && !seen.has([forward.row, forward.col])) {
      const seenClone = seen.clone();
